...
class Phi(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.calculate_entropy:
            ...
            self.reg_loss = torch.clamp(self.w.abs(), min=1e-5).sum(-1).mean()
        return output
...
class LogicalLayer(nn.Module):
    def __init__(self, in_features, out_features, use_phi=False, use_weight_sigma=True, use_weight_exp=True):
        super().__init__()

...
class AromaticCarbon(Dataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        super(AromaticCarbon, self).__init__(root, transform, pre_transform, pre_filter)
        self.data_df = pd.read_csv(root)
        self.data_df['mol'] = self.data_df['smiles'].apply(Chem.MolFromSmiles)

        # Global atom and bond types across all molecules
        atom_type_set = set()
        bond_type_set = set()
    ...
    def get(self, idx):
        ...
        # Edge indices and one-hot bond features
        edge_index = []
        edge_attr = []
        ...
        # Node labels: per-atom aromaticity
        y = torch.tensor(

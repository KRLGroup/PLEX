        ...
        try:
            ...
            with torch.no_grad():
                if data.edge_attr is None:
                    layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float().to(device), data.edge_index.to(device), None, data.batch.to(device), tau=1000)
                else:
                    layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device), data.batch.to(device), tau=1000)
            ...
            if train_full == False or (train_full == True and  (only_teacher == 1 or only_teacher == 2)):
                ...
                if data.edge_attr is not None and model_tell.negative_concatenate!=0:
                    if model_tell.phi_edge is not None:
                        ...
                        edge_attr = torch.hstack([edge_attr, 1-edge_attr]).float().to(device)
                    else:
                    ...
                    if model_tell.phi_edge is not None:
                        edge_attr = data.edge_attr.float().to(device)
                        edge_attr = model_tell.phi_edge(edge_attr).float().to(device)
                    else:
                        edge_attr = data.edge_attr.float().to(device)
...
def train_seed(dataset_name, baseline_path, args, seed, device):
    ...
    if dataset_name in ['PROTEINS', 'Ba2Motifs', 'BaMultiShapes']:
    ...
    for epoch in range(args['epochs']):
        train_loss, train_acc = train_epoch(model, model_tell, train_loader, device, optimizer, num_classes, train_full=epoch>=args['warmup_epochs'], conv_reg=args['conv_reg'], fc_reg=args['fc_reg'], only_teacher=args['only_teacher'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conv_reg',      default=0.01,      type=float,   help='Conv layer regularization')
    parser.add_argument('--fc_reg',        default=0.01,      type=float,    help='Last layer regularization')
    parser.add_argument('--dropout',       default=0.0,       type=float,   help='Dropout')
    parser.add_argument('--negative_concatenate', default=1, type=int, help='Use negative concatenation')

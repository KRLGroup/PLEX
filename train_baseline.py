...
def train_epoch(model, loader, device, optimizer, num_classes):
    ...
    for data in loader:
        ...
        if data.edge_attr is None:
            out = model(data.x.float().to(device), data.edge_index.to(device), None, data.batch.to(device), tau=1)
        else:
            out = model(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device), data.batch.to(device), tau=1)
...
def train_seed(dataset_name, args, seed, device):
    ...
    print(f'Num features: {num_features}, Num edge features: {num_features_edge}, Num classes: {num_classes}')
    ...
    model = GIN(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=args['hidden_dim'], num_layers=args['num_layers'], nogumbel=args['nogumbel'],
...
def eval_seed(dataset_name, args, seed, device):
    ...
    model = GIN(num_features=num_features, num_classes=num_classes, num_features_edge=num_features_edge, hidden_dim=args['hidden_dim'], num_layers=args['num_layers'], nogumbel=args['nogumbel'],
                edge_once=args['edge_once'], layer_double=args['layer_double']).to(device)

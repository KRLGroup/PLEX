...
def train_epoch(model, model_tell, data, mask, device, optimizer, num_classes, train_full=True, conv_reg=0.001, fc_reg=0.01, only_teacher=0):
    model_tell.train()
    data = data.to(device)
    ...
    with torch.no_grad():
        if data.edge_attr is None:
            layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float(), data.edge_index, None, tau=1000)
        else:
            layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float(), data.edge_index, data.edge_attr.float(), tau=1000)
    ...
    if train_full == False or (train_full == True and  (only_teacher == 1 or only_teacher == 2)):
        ...
        for i, (layer, layer_x, layer_y, layer_y_interm) in enumerate(zip(model_tell.convs, layers_x[:-1], layers_y[:-1], layers_y_interm)):
            ...
            total_loss[i] += layer_loss.item() / data.num_nodes
            total_correct[i] += ((layer_out >= 0.5).long() == layer_y).sum().item() / (layer_y.shape[-1]*data.num_nodes)
        ...
        if only_teacher == 2:
            ...
            loss_fc_teacher = F.binary_cross_entropy(out[mask, :].reshape(-1), torch.nn.functional.one_hot(y[mask], num_classes=num_classes).float().reshape(-1)) + F.nll_loss(F.log_softmax(out[mask, :], dim=-1), y[mask].long())
        #print('FC layer loss teacher:', loss_fc_teacher.item())
        loss += loss_fc_teacher

    if train_full:
        if data.edge_attr is not None:
            out = model_tell(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device))
        else:
            out = model_tell(data.x.float().to(device), data.edge_index.to(device), None)
        
        pred = out.argmax(-1)

        loss_fc_teacher_full= F.binary_cross_entropy(out[mask, :].reshape(-1), torch.nn.functional.one_hot(y[mask], num_classes=num_classes).float().reshape(-1)) + F.nll_loss(F.log_softmax(out[mask, :], dim=-1), y[mask].long())
    ...
    optimizer.step()
    total_loss[-1] += loss.item() / data.num_nodes
...
def test_epoch(model, data, mask, device):
    ...
    y = data.y.reshape(-1)
    if data.edge_attr is None:
        pred = model(data.x.float(), data.edge_index, None, tau=1000).argmax(-1)
    else:
        pred = model(data.x.float(), data.edge_index, data.edge_attr.float(), tau=1000).argmax(-1)
...
def train_seed(dataset_name, baseline_path, args, seed, device):
    ...
    model = GIN(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=baseline_args['hidden_dim'], num_layers=baseline_args['num_layers'], nogumbel=baseline_args['nogumbel'],
    ...
    for epoch in range(args['epochs']):
        for batch in train_loader:
            train_loss, train_acc = train_epoch(model, model_tell, batch, batch.train_mask, device, optimizer, num_classes, train_full=epoch>=args['warmup_epochs'], conv_reg=args['conv_reg'], fc_reg=args['fc_reg'], only_teacher=args['only_teacher'])

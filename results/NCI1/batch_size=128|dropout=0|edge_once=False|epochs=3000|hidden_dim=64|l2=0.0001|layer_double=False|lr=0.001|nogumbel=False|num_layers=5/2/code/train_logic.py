        ...
        try:
            ...
            with torch.no_grad():
                if data.edge_attr is None:
                    layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float().to(device), data.edge_index.to(device), None, data.batch.to(device))
                else:
                    layers_x, layers_y, layers_y_interm = model.forward_e(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device), data.batch.to(device))
            
            loss = 0
            last_layer_out = None
            
            if data.edge_attr is not None and model_tell.negative_concatenate!=0:
                edge_attr = torch.hstack([data.edge_attr, 1-data.edge_attr]).float().to(device)
            ...
            for i, (layer, layer_x, layer_y, layer_y_interm) in enumerate(zip(model_tell.convs, layers_x[:-1], layers_y[:-1], layers_y_interm)):
                ...
                if i==0 and not model_tell.input_binary:
                    layer_x = model_tell.phi(layer_x)
            ...
            if train_full:
                ...
                out = model_tell(data.x.float().to(device), data.edge_index.to(device), data.edge_attr, data.batch.to(device))
                pred = out.argmax(-1)
                loss += F.binary_cross_entropy(out.reshape(-1), torch.nn.functional.one_hot(y, num_classes=num_classes).float().reshape(-1)) + F.nll_loss(F.log_softmax(out, dim=-1), y.long())
...
def train_seed(dataset_name, baseline_path, args, seed, device):
    ...
    print('Num features:', num_features, 'Num edge features:', num_features_edge, 'Num classes:', num_classes)
    ...
    model = GIN(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=baseline_args['hidden_dim'], num_layers=baseline_args['num_layers'], nogumbel=baseline_args['nogumbel']).to(device)
    ...
    if dataset_name=='PROTEINS':
        model_tell = GINTELL(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=baseline_args['hidden_dim'], num_layers=baseline_args['num_layers'],
                              negative_concatenate=args['negative_concatenate'], edge_again=args['edge_again'], input_binary=False).to(device)
    else:
        model_tell = GINTELL(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=baseline_args['hidden_dim'], num_layers=baseline_args['num_layers'],
                                negative_concatenate=args['negative_concatenate'], edge_again=args['edge_again']).to(device)
...
def eval_seed(dataset_name, baseline_path, args, seed, device):
    ...
    model = GIN(num_features=num_features, num_classes=num_classes, num_features_edge=num_features_edge, hidden_dim=baseline_args['hidden_dim'], num_layers=baseline_args['num_layers'], nogumbel=baseline_args['nogumbel']).to(device)
    model.load_state_dict(torch.load(os.path.join(baseline_path, 'best.pt'), map_location='cpu'))
    model.eval()
    for p in model.parameters():
        p.requires_grad_ = False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--negative_concatenate', default=0, type=int, help='Use negative concatenation')

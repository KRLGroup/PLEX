...
def train_epoch(model, data, mask, device, optimizer, num_classes):
    ...
    if data.edge_attr is None:
        out = model(data.x.float().to(device), data.edge_index.to(device), None, tau=1)[mask,:]
    else:
        out = model(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device), tau=1)[mask,:]
...
def test_epoch(model, data, mask, device):
    ...
    y = data.y[mask].to(device)
    if data.edge_attr is None:
        out = model(data.x.float().to(device), data.edge_index.to(device), None, tau=1000)[mask,:]
    else:
        out = model(data.x.float().to(device), data.edge_index.to(device), data.edge_attr.float().to(device), tau=1000)[mask,:]
...
def train_seed(dataset_name, args, seed, device):
    ...
    model = GIN(num_features=num_features, num_features_edge=num_features_edge, num_classes=num_classes, hidden_dim=args['hidden_dim'], num_layers=args['num_layers'], 
                nogumbel=args['nogumbel'], edge_once = args['edge_once'], layer_double=args['layer_double']).to(device)
    ...
    max_patience = 200
    patience = 0
    #print("Train:", dataset.data.train_mask.sum(), "Val:", dataset.data.val_mask.sum(), "Test:", dataset.data.test_mask.sum())

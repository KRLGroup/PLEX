# """
# Script per analizzare le attivazioni dalle regole salvate.

# Usa i dati 'all_nodes_mask' già presenti in all_rules per calcolare
# la percentuale di nodi attivati per ogni regola.
# """
...
#         # mask è un vettore 1D: [num_nodes] o [num_edges]
#         # Ogni entry è True/False
...
#     # Salva CSV
#     output_csv = f'rule_activations_{dataset_name}.csv'
...
#     print("\nSUMMARY STATISTICS BY LAYER AND SUBLAYER:")
#     summary = df.groupby(['layer', 'sublayer']).agg({
#         'feature': 'count',
#         'pct_activated_nodes': ['mean', 'std', 'min', 'max']
#     })
#     summary.columns = ['num_rules', 'mean_pct', 'std_pct', 'min_pct', 'max_pct']
#     print(summary.round(2))
    
#     # Distribuzione generale
#     print("\nOVERALL STATISTICS:")
#     print(f"  Total rules: {len(df)}")
#     print(f"  Average activation: {df['pct_activated_nodes'].mean():.2f}%")
#     print(f"  Std deviation: {df['pct_activated_nodes'].std():.2f}%")
#     print(f"  Min activation: {df['pct_activated_nodes'].min():.2f}%")
#     print(f"  Max activation: {df['pct_activated_nodes'].max():.2f}%")
    
#     return df


# def create_activation_histogram(df, dataset_name, save_path='activation_histogram.png', num_layers=5):
#     """
#     Crea l'istogramma delle attivazioni per layer, separando lambda_0 e lambda_1.
    
#     Args:
#         df: DataFrame con le attivazioni
#         dataset_name: Nome del dataset
#         save_path: Path dove salvare il plot
#         num_layers: Numero totale di layer da mostrare (default: 5)
#     """
#     import matplotlib.pyplot as plt
    
#     # 2 righe: una per lambda_0, una per lambda_1
#     # num_layers colonne
#     fig, axes = plt.subplots(2, num_layers, figsize=(4*num_layers, 6))
...
#     for row_idx, sublayer in enumerate(sublayer_names):
#         for col_idx in range(num_layers):
#             layer = col_idx  # layer interno (0-based)
#             ax = axes[row_idx, col_idx]
            
#             # Filtra per layer e sublayer
#             layer_data = df[(df['layer'] == layer) & (df['sublayer'] == sublayer)]['pct_activated_nodes'].values
            
#             if len(layer_data) == 0:
#                 # Grafico vuoto - niente testo "No data"
#                 ax.set_xlim(0, 100)
#                 ax.set_ylim(0, 1)
#             else:
...
#             # Griglia sempre presente
#             ax.grid(True, alpha=0.3)
            
#             # Labels
#             if row_idx == 1:  # Ultima riga
#                 ax.set_xlabel('% Activated Nodes', fontsize=9)
#             if col_idx == 0:  # Prima colonna
#                 ax.set_ylabel('# Rules', fontsize=9)
            
#             # Titolo solo sulla prima riga - MOSTRA LAYER+1
#             if row_idx == 0:
#                 ax.set_title(f'Layer {layer + 1}', fontsize=10)
            
#             # Label sublayer a sinistra
#             if col_idx == 0:
#                 ax.text(-0.3, 0.5, f'λ{row_idx}', 
#                        transform=ax.transAxes, rotation=90, 
#                        va='center', ha='center', fontsize=10, fontweight='bold')
    
#     plt.suptitle(f'{dataset_name} - Rule Activations', fontsize=14, fontweight='bold')
#     plt.tight_layout()
...
# def count_rules_per_layer(all_rules_path, dataset_name='dataset', num_layers=5):
#     """
#     Conta il numero di regole per ogni layer e sublayer.
    
#     Args:
#         all_rules: Dictionary con le regole estratte
#         dataset_name: Nome del dataset
#         num_layers: Numero totale di layer
    
#     Returns:
#         DataFrame con colonne: layer, sublayer, num_rules
#     """
#     print(f"Loading all_rules from: {all_rules_path}")
#     with open(all_rules_path, 'rb') as f:
#         all_rules = pickle.load(f)
...
#     # Salva CSV
#     output_csv = f'rule_counts_{dataset_name}.csv'
#     df.to_csv(output_csv, index=False)
#     print(f"\nSaved rule counts to {output_csv}")
#     print("\nRule counts:")
#     print(df)
...
# def plot_rule_counts(df, dataset_name, save_path='rule_counts.png'):
#     """
#     Crea un grafico a barre del numero di regole per layer.
#     Distingue lambda_0 e lambda_1 con barre affiancate.
    
#     Args:
#         df: DataFrame con colonne: layer, sublayer, num_rules
#         dataset_name: Nome del dataset
#         save_path: Path dove salvare il plot
#     """
#     import matplotlib.pyplot as plt
...
#     lambda_0_counts = df[df['sublayer'] == 'lambda_0'].sort_values('layer')['num_rules'].values
#     lambda_1_counts = df[df['sublayer'] == 'lambda_1'].sort_values('layer')['num_rules'].values
...
#     ax.set_title(f'{dataset_name} - Number of Rules per Layer', fontsize=14, fontweight='bold')
#     ax.set_xticks(x)
#     # MOSTRA LAYER+1 (da 1 a 5 invece di 0 a 4)
...
#     # Salva
#     plt.savefig(save_path, dpi=300, bbox_inches='tight')
#     print(f"Saved rule count plot: {save_path}")
#     plt.close()


# all_rules_path = '/home/palu001/LogiX-Me/explanations/Ba2Motifs/0/classe_1/all_rules.pkl' 
# dataset_name = 'Ba2Motifs'

# df = analyze_activations_from_all_rules(all_rules_path, dataset_name)
# create_activation_histogram(df, dataset_name, save_path=f'activation_histogram_{dataset_name}.png',num_layers=3)
# counts_df = count_rules_per_layer(all_rules_path, dataset_name, 3)
# plot_rule_counts(counts_df, dataset_name, 
#                 save_path=f'rule_counts_{dataset_name}.png')
"""
